...
void setFillTheBlank()//void setup for fill the blank
{
  ...
  //{
    t[0]=new textbox(location_and_size[1][0][0],
...
void buttonAction()
{
  ...
  for(int i=0; i<t.length; i++)
    {
      res[i]=t[0].check();//getting the results from the object
...
void switching()//checking what texbox was chosen by user
{
  if(mousePressed)
  {
    for(int i=0; i<t.length; i++)
    {
      if(mouseX>=t[0].Xmin && mouseX<=t[0].Xmax && mouseY>=t[0].Ymin && mouseY<=t[0].Ymax)
      {
       boxnumber=i+1;
      }
  }
